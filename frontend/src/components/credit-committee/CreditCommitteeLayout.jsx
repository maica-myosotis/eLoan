...
export default function CreditCommitteeLayout() {
  ...
  useEffect(() => {
    ...
    if (!currentUser || currentUser.role !== 'Credit Committee') {
      navigate('/');
      return;
    }
    setUser(currentUser);
  ...
  const fetchUnreadCount = async () => {
    try {
      const data = await creditCommitteeService.getUnreadCount();
  ...
  const navItems = [
    ...
    { path: '/credit-committee/decisions', icon: 'check-circle', label: 'Decision History' },
    { path: '/credit-committee/reports', icon: 'bar-chart-2', label: 'Reports' },
    { path: '/credit-committee/notifications', icon: 'bell', label: 'Notifications', badge: unreadCount },
    { path: '/credit-committee/settings', icon: 'settings', label: 'Settings' },
  ];
        ...
        <div style={{
          ...
          {sidebarOpen && (
            <div>
              ...
              <div style={{ color: primaryColor, fontSize: '0.75rem', fontWeight: 500 }}>Credit Committee</div>
            </div>
          )}
        </div>
