...
export default function Settings() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [profileRes, prefsRes] = await Promise.all([
        treasurerService.getProfile(),
        treasurerService.getNotificationPreferences(),
      ]);
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      setSaving(true);
      await treasurerService.changePassword(
  ...
  const handlePreferenceToggle = async (key) => {
    ...
    try {
      await treasurerService.updateNotificationPreferences(updatedPrefs);
  ...
  const handleLogoutEverywhere = async () => {
    ...
    try {
      setSaving(true);
      await treasurerService.logoutEverywhere();
  ...
  const handleDeactivateAccount = async () => {
    ...
    try {
      setSaving(true);
      await treasurerService.deactivateAccount(deactivatePassword);
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem' }}>
              <div
                ...
                style={{
                  ...
                  height: '80px',
                  borderRadius: '50%',
                  background: profile?.profile_picture ? `url(${profile.profile_picture}) center/cover` : primaryColor,
                  display: 'flex',
                  alignItems: 'center',
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', padding: '0.75rem', background: '#f9fafb', borderRadius: '0.5rem' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>
                      Receive email alerts for forwarded applications and updates
                    </div>
                  </div>
                  <ToggleSwitch
